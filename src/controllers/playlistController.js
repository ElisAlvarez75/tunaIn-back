  ...
  router.put('/:id', (req, res) => {
    const updateData=  {};
     ...
     if (req.body.description) {
      ...
      Playlist.findByIdAndUpdate(req.params.id, updateData,{ 'new': true })
      .then((result) => {
        if (result) {
          res.status(200).json(result);
        } else {
  ...
  router.put('/:playlistId/podcast', (req, res) => {
    if (req.body.list) {
      ...
      Podcast.find({'_id' : { $in : idArray }}).then((result) =>{
       ...
       Playlist.findByIdAndUpdate(req.params.playlistId, {$push : { list: array }}, { new:true })
       .then((r) => {
         if (r) {
  ...
  router.patch('/:playlistId/podcast', (req, res) => {
    if (req.body.list) {
      ...
      const array = result.map(podcast => podcast._id);
        ...
        Playlist.updateMany( {'_id':req.params.playlistId}, {$pull : {list: array}})
         .then((r) => {
           if (r) {
             res.status(200).json(r);
           } else {
